...
{  
	...
	foreach($select as $reg)
	{
      ...
      $retorno .= "<table class=\"tabela\" width=\"100%\" cellpadding=\"8\" border=\"1\">
		...
		$select = "SELECT tb_alunos.aluno_nome,tb_alunos.aluno_cpf, tb_alunos.aluno_email, tb_matricula.matricula_status_pg, tb_matricula.data_pg FROM tb_alunos INNER JOIN tb_matricula ON tb_alunos.aluno_id = tb_matricula.matricula_aluno_id and matricula_curso_id = ? and matricula_status_pg = 'PAGAMENTO APROVADO'";
		$result = $conexao->prepare($select);
		$result->bindValue(1, $curso_id);
		$result->execute();
		$select = $result->fetchAll();
		...
		{
			echo $mensagem = date("d/m/Y",strtotime($reg['data_pg']));				
		}
