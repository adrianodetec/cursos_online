...
{  
	...
	foreach($select as $reg)
	{
      ...
      $retorno .= "<table class=\"tabela\" width=\"100%\" cellpadding=\"8\" border=\"1\">
		...
		$select = "SELECT tb_alunos.aluno_nome,tb_alunos.aluno_cpf, tb_alunos.aluno_email, tb_alunos.aluno_profissao, tb_matricula.foto, tb_matricula.matricula_status_pg FROM tb_alunos INNER JOIN tb_matricula ON tb_alunos.aluno_id = tb_matricula.matricula_aluno_id and matricula_curso_id = ?";	
		$result = $conexao->prepare($select);
		$result->bindValue(1, $curso_id);
		$result->execute();
		$select = $result->fetchAll();
		...
		foreach($select as $reg):		 
			...
			$retorno .= "<td align=\"center\" class=\"converte_maiusculo\">{$reg['aluno_profissao']}</td>";			
