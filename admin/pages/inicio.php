...
$dados1 = $conexao->prepare($select);
$dados1->bindValue(1, $session_aluno_id);	
$dados1->execute();
$inscricaoPendentes = $dados1->rowCount();$select = "select * from tb_matricula where matricula_aluno_id = ? and matricula_status_pg = 'PAGAMENTO APROVADO'";$dados2 = $conexao->prepare($select);@$dados2->bindValue(1, $session_aluno_id);	$dados2->execute();$inscricaoAprovadas = $dados2->rowCount();$query = $conexao->prepare("select SUM(valor_pago) as valorSoma from tb_matricula where matricula_aluno_id = ? and matricula_status_pg = 'PAGAMENTO APROVADO'");$query->bindValue(1, $session_aluno_id);$query ->execute();$result = $query->fetchAll();foreach ($result as $obj){  $valor = $obj['valorSoma'];}
?>
				 ...
				 <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                   <div class="info-box hover-zoom-effect">
                        ...
                        <div class="content">
                            ...
                            <div class="number"><?php print_r($valor)//echo number_format($valor_investido,2) ?></div>
                        </div>
                    </div>
                </div>				
